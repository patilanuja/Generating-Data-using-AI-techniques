...
def show_cifar_images(images):
    images = torch.reshape(
        images, [images.shape[0], -1]
    )  # images reshape to (batch_size, D)
    sqrtn = int(math.ceil(math.sqrt(images.shape[0])))
    sqrtimg = int(math.ceil(math.sqrt(images.shape[1]//3)))
    ...
    for i, img in enumerate(images):
        ...
        ax.set_yticklabels([])
        ax.set_aspect("equal")
        reshape=np.transpose(img.reshape([3,sqrtimg,sqrtimg]),(1,2,0))
        output = np.array((reshape+1)*(255/2),np.int32)
        plt.imshow(output)
    return
