...
def generator_cifar(noise_dim=NOISE_DIM):
    """
...
def build_dc_classifier_cifar():
    ...
    model = nn.Sequential(
        ...
        nn.MaxPool2d(kernel_size=2, stride=2),
        nn.Flatten(),
        nn.Linear(in_features=4*4*100 , out_features=4*4*64),  #nn.Linear(in_features=4*4*64 , out_features=4*4*64),
        nn.LeakyReLU(0.01),
...
def build_dc_generator_cifar(noise_dim=NOISE_DIM):
    """
