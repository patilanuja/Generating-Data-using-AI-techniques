...
def discriminator_cifar():
  ...
  # Replace "pass" statement with your code
    model = nn.Sequential(
    nn.Flatten(),
    nn.Linear(1024*3, 256),  # 1st Fully-Connected layer.
    nn.LeakyReLU(0.01),
    nn.Linear(256, 256),  # 2nd Fully-Connected layer.
    nn.LeakyReLU(0.01),
    nn.Linear(256, 1) , # 3rd Fully-Connected layer. 
...
def generator_cifar(noise_dim=NOISE_DIM):
    ...
    nn.Linear(1024, 1024),       # 2nd Fully-Connected layer.
    nn.ReLU(),
    nn.Linear(1024, 1024*3),        # 3rd Fully-Connected layer.
    nn.Tanh()
  )
    ############################################################################
    #                             END OF YOUR CODE                             #
...
def discriminator_loss(logits_real, logits_fake):
  ...
  Returns:
  - loss: PyTorch Tensor containing (scalar) the discriminatorloss for the discriminator.
  """
  loss = None
  ##############################################################################
